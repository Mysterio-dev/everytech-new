    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".js-menu")) {
                ...
                document.addEventListener("click", (function(e) {
                    if (bodyLockStatus && e.target.closest(".js-menu-close")) {
                        bodyLockToggle();
                        document.documentElement.classList.toggle("menu-open");
                    }
                }));
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            ymaps.ready((function() {
                var myMap = new ymaps.Map("map", {
                    ...
                    zoom: 15,
                    controls: []
                }), MyIconContentLayout = ymaps.templateLayoutFactory.createClass("<div>$[properties.iconContent]</div>"), myPlacemark = new ymaps.Placemark([ 55.782543, 49.120074 ], {
                    ...
                    iconImageHref: "img/icons/office_2.png",
                    iconImageSize: [ 80, 100 ],
                    iconImageOffset: [ -5, -38 ],
                    hideIconOnBalloonOpen: false,
                    balloonCloseButton: false,
                    balloonOffset: [ 17, -20 ]
                }), myPlacemarkWithContent = new ymaps.Placemark([ 55.787943, 49.130174 ], {
                    balloonContent: '<div class="map__content">Офис в городе на Бутлерова 21</div>',
                    balloonContentFooter: '<div class="map__footer"><a href="" class="map__link"><span>Подробнее</span></a></div>'
                }, {
                    iconLayout: "default#imageWithContent",
                    iconImageHref: "img/icons/office.png",
                    iconImageSize: [ 80, 100 ],
        ...
        const script_select = (el, all = false) => {
            el = el.trim();
            if (all) return [ ...document.querySelectorAll(el) ]; else return document.querySelector(el);
        };
        const script_on = (type, el, listener, all = false) => {
            if (all) script_select(el, all).forEach((e => e.addEventListener(type, listener))); else script_select(el, all).addEventListener(type, listener);
        };
        window.addEventListener("load", (() => {
            let portfolioContainer = script_select(".portfolio__row");
            if (portfolioContainer) {
                ...
                let portfolioFilters = script_select("#portfolio-flters li", true);
                script_on("click", "#portfolio-flters li", (function(e) {
                    e.preventDefault();
                    portfolioFilters.forEach((function(el) {
                        el.classList.remove("_filter-active");
                    }));
                    this.classList.add("_filter-active");
